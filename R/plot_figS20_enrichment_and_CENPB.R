...
pC<-ggplot(SUB, aes(x=CENPB, y=value, fill=NonB, color=NonB))+
  ...
  scale_color_manual(breaks=c("APR", "DR", "STR", "IR", "MR", "TRI", "G4", "Z"), values=nonBcol)+
  geom_hline(yintercept=1, linetype="dashed", color = "red", linewidth=1)+
  labs(x="CENP-B", y="Fold-enrichment") +
  theme(panel.background = element_rect(fill = 'white', colour="white"),
        strip.background = element_rect(fill = 'lightgray', colour="white"),
        strip.text = element_blank(),
        panel.grid.major = element_line(colour = 'beige'),
...
pcomb <- pA +  pB +  pC + plot_layout(heights = c(5, 5, 5)) 
pcomb


outfile=paste(plotdir,"FigS20_ABC_CENPB.jpg", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 600,limitsize = TRUE,width=6,height=9)
