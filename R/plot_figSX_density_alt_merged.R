...
# Input files 
density_file <- "densities/alt_6sp_merged.txt"
lengths_file <- "densities/alt_6sp_lengths.txt"
centro_file <-"densities/alt_6sp_centromeres.txt"
outfile <- "plots/FigureSX_alternative_densities.jpg"
windsize=100000
...
comb_tib <- dens_tib %>% left_join(len_tib) %>% left_join(cen_tib) %>% 
  ...
  mutate(maxlen=case_when(Species=="chimp" ~ 260000000,
                          Species=="borang" ~ 260000000,
                          Species=="gorilla" ~ 300000000,
                          TRUE ~ 280000000))
comb_tib$Species <- factor(comb_tib$Species, levels=c("chimp", "bonobo", "gorilla", "borang", "sorang"))
...
### ~~~~~~~~~~~~~~~~~~ Gorilla
sub_tib3 <- merged_tib %>% filter(Species=="gorilla")
...
### ~~~~~~~~~~~~~~~~~~ borang
sub_tib4 <- merged_tib %>% filter(Species=="borang")
...
### ~~~~~~~~~~~~~~~~~~ Sorang
sub_tib5 <- merged_tib %>% filter(Species=="sorang")
...
pcomb <- p1 + p2 + plot_spacer() + p3 + p4 + p5 + plot_layout(widths = c(1, 1, 1), heights= c(1,1)) 
pcomb
# Save to figure 
