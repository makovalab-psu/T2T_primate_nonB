...
statfile=paste("new_sequence/human_pri/stat_table.tsv", sep="")
...
RES_TABLE %>% as_tibble() %>% mutate(sign=case_when(pval<0.05/27 ~ "yes",
                               TRUE ~ "no")) %>% print(n=27)
################################################################################
# SIMILAR FOR THE RESAMPLED SETS 
resamfile=paste("new_sequence/human_pri/resample.stat_table.tsv", sep="")
