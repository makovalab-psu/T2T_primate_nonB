...
GROUP$Motif <- factor(GROUP$Motif, levels=c("APR", "DR", "STR", "IR", "MR", "G4", "Z"))
...
################################################################################
# PLOT PART1: REPEAT ENRICHMENT

p1<-ggplot(REPDATA, aes(PrintName, forcats::fct_rev(Motif))) +
  geom_tile(aes(fill=logval), color = "white",lwd = 0.3,linetype = 1) +
  facet_grid(~Class, space="free", scales="free_x") +
  geom_point(data=REPDATA %>% filter(Mark=="Yes"), aes(x=PrintName, y=8), color="black", fill="black", inherit.aes = FALSE, pch=25) +
