################################################################################
### R code for plotting OVERLAP OF NON-B DNA TYPES used for Figure 3
### Written by Linnéa Smeds Nov-12-2024
...
mergetib <- tibA %>% bind_rows(tibX) %>% bind_rows(tibY) %>% 
  filter(Bp>10000) %>% arrange(desc(Bp))

pup<-ggplot(mergetib, aes(x=Types, fill=Chr)) +
...
#  scale_fill_manual(values=c("#1964B0","#7BB0DF", "#B6DBFF"))+
  ...
  theme(panel.background = element_rect(fill = 'white', colour="white"),
        ...
        strip.text = element_markdown(hjust=0, size=14, face="bold"),
...
ppw<-ggplot(tibP, aes(x=NB2, y=forcats::fct_rev(NB1), fill=Frac)) +
...
outfile=paste(plotdir,"Fig3_overlap_",species,".png", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 600,limitsize = TRUE,width=12,height=12)
