...
for (i in 1:6) {
  ...
  p<-ggplot(sub_tib, aes(Start/1000000, fct_rev(NonB), fill=fct_rev(NonB))) +
    ...
    geom_line(data=sub_sat, aes(x=Sat_start/1000000, y=10, color=FullName), linewidth=3, inherit.aes = FALSE) + 
    geom_rect(aes(xmin=(Cen_mid-5000000)/1000000, ymin=10, ymax=10, xmax=(Cen_mid+5000000)/1000000), color="white", fill="white", linewidth=5, show.legend = FALSE) +
    geom_rect(data=sub_sat, aes(xmin=Sat_start/1000000, ymin=10, ymax=10, xmax=Sat_stop/1000000, color=FullName), fill="white", linewidth=4, inherit.aes = FALSE, show.legend = FALSE) +
  ...
  output<- paste("plots/FigS17_density_centromere_",sp[i],".png", sep="")
  if(i==3){
