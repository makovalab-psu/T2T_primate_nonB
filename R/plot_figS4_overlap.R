...
# plot_fig3_overlap.R
# Note 3: The legend showing number of overlapping types in the bottom of the 
# upset plot was added manually
...
for (i in (1:length(spvect))) {
  ...
  ppw<-ggplot(DATA2, aes(x=NB2, y=fct_rev(NB1), fill=Frac)) +
    ...
    geom_text(aes(label=round(Frac*100, digits=2), color=textcol), size=4, show.legend=FALSE) +
  ...
  outfile=paste(plotdir,"FigS4_overlap_",species,"_testSize.png", sep="")
