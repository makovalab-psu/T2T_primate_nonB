...
cen_tib$NonB <- factor(cen_tib$NonB, levels=c("APR", "DR", "STR", "IR", "MR", "G4", "Z"))
...
# 90 groups remain (incl sec haplo: 158)
...
# 57 groups (incl alt haplo: 100)

################################################################################
# MAKE TABLE WITH SIGNIFICANT (OR ALL) ENRICHMENT, FOR EXPORT

EXPORT <- cen_tib %>% #filter(value>1) %>% 
  select(Species, Chr, NonB, Significance, PrintNum) %>% 
  pivot_wider(id_cols=c(Species,Chr), values_from=PrintNum, names_from=(NonB))
write.table(EXPORT, quote=FALSE, sep="\t", file = "centromeres/pri_6sp_significance.tsv")
