#################################################################################
### R code for plotting enrichment in centromeres for 6 species combined, and 
### statistical test for comparing centromeres and background
...
pcomb
outfile=paste(plotdir,"Fig7_Enrichment_centromeres.png", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 600,limitsize = TRUE,width=9,height=12)
outfile=paste(plotdir,"Fig7.tiff", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 600,limitsize = TRUE,width=8,height=11, device = "tiff", compression = "lzw")
...
# 79 groups remain (incl sec haplo: 140)
...
# 54 groups (incl alt haplo: 94)

################################################################################
# MAKE TABLE WITH SIGNIFICANCE, INCLUDE SF AND CENPB INFO (HALF OF TABLE S5) 

SF_file="centromeres/pri_6sp_SF_merged.tsv"
tib_SF<-SF_file %>% read.table(header=TRUE) %>% as_tibble() %>% distinct() %>% mutate(Hap="Pri")
cenpb_file="centromeres/pri_6sp_cenpb.tsv"
...
write.table(EXPORT, row.names = FALSE, quote=FALSE, sep="\t", file = "centromeres/pri_6sp_significance_with_SF_CENPB.tsv")
