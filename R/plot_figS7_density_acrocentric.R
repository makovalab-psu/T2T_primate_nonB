...
for (i in 1:6) {
  ...
  sub_tib$NonB <- factor(sub_tib$NonB, levels=c("APR", "DR", "STR", "IR", "MR", "TRI", "G4", "Z"))
  ...
  pacr<-ggplot(sub_tib, aes(Start/1000000, fct_rev(NonB), fill=fct_rev(NonB))) +
    ...
    labs(x="Mb", y="") +
    geom_line(data=sub_sat %>% filter(Sat!="rdna"), aes(x=Sat_start/1000000, y=10, color=FullName), linewidth=3, inherit.aes = FALSE) + 
    geom_rect(aes(xmin=0, ymin=10, ymax=10, xmax=Cen_end/1000000), color="white", fill="white", linewidth=5, show.legend = FALSE) +
    geom_rect(data=sub_sat %>% filter(Sat!="rdna"), aes(xmin=Sat_start/1000000, ymin=10, ymax=10, xmax=Sat_stop/1000000, color=FullName), fill="white", linewidth=3, inherit.aes = FALSE, show.legend = FALSE) +
    ...
    geom_segment(data=sub_sat %>% filter(Sat=="rdna"), aes(x=Sat_start/1000000, xend=Sat_stop/1000000, y=9.8, yend=9.8), color="black", linewidth=0.4, inherit.aes = FALSE, arrow=arrow(type="closed", length = unit(0.07, "cm"))) +
  ...
  pacr
  output<- paste("plots/FigS7_acrocentric_p-arm_",sp[i],".png", sep="")
  if(i==3){
