...
cat T2T_primate_nonB/helpfiles/alternative_species_list.txt | while read -r trivial latin filename;
do
    echo "Creating directory for $trivial.."
...
# ~~~~~~ NOT SURE I WILL NEED THE FOLLOWING 
# Start by putting the total length in a file
#cat T2T_primate_nonB/helpfiles/primary_species_list.txt | grep "human" |while read -r trivial latin filename;
#do
 # rm -f summaries/${trivial}_nonB_len.txt
 # for chr in "autosomes" "chrX" "chrY"
 # do
 #   for n in "APR" "DR" "GQ" "IR" "MR" "STR" "Z" "all"
 #   do
 #     #echo $n
 #     nonB=`awk '{sum+=$3-$2}END{print sum}' nonB_annotation/$latin/${chr}_$n.bed`
 #     echo $chr" "$n" "$nonB >>summaries/${trivial}_nonB_len.txt
 #   done
 # done
# done
# ~~~~~~ 

# For the onset plot, one need a list with all bases and non-B types  
mkdir overlap
for sp in "human"
do 
    echo '#!/bin/bash      
    echo "Chr Pos Type" >overlap/primary_'$sp'.txt
    for nb in "APR" "DR" "GQ" "IR" "MR" "STR" "Z" "all"
    do
